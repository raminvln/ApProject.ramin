...
class PhotoDetailPage extends StatefulWidget {
  ...
  const PhotoDetailPage({super.key, required this.photo, this.fromAlbum});
                    ...
                    onDone: (albums) {
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              'Photo added to ${albums.length} album(s)',
                            ),
                      ...
                      onDone: (albums) {
                        if (mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(
                                'Photo copied to ${albums.length} album(s)',
                              ),
                              backgroundColor: Colors.green,
                              behavior: SnackBarBehavior.floating,
                            ),
                      ...
                      onDone: (albums) {
                        if (mounted) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(
                                'Photo moved to ${albums.length} album(s)',
                              ),
                              backgroundColor: Colors.green,
                              behavior: SnackBarBehavior.floating,
                            ),
                ...
                _buildOptionItem(
                  ...
                  onTap: () {
                    ...
                    _showConfirmationDialog(
                      title: 'Remove Photo',
                      content:
                          'Remove "${widget.photo.title}" from ${widget.fromAlbum}?',
                      onConfirm: () {
                      ...
                      decoration: BoxDecoration(
                        ...
                        image: DecorationImage(
                          ...
                          onError: (error, stackTrace) {
                            // اگر عکس نبود، رنگ پس‌زمینه می‌افته
                          },
                        ),
                      ),
                    ),
