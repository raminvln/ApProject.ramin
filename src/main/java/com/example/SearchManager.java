...
public class SearchManager {
    ...
    public static List<Picture> searchByAlbum(String albumName, User user) {
        return user.getPictures().stream()
                .filter(picture -> picture.getAlbums().stream()
                        .anyMatch(album -> album.getName().toLowerCase().contains(albumName.toLowerCase())))
    ...
    public static List<Picture> searchByComment(String commentText, User user) {
        return user.getPictures().stream()
                .filter(picture -> picture.getComments().stream()
                        .anyMatch(comment -> comment.getText().toLowerCase().contains(commentText.toLowerCase())))
                .collect(Collectors.toList());
    }
    ...
    public static List<Picture> sortByDateTime(User user) {
        return user.getPictures().stream()
                .sorted((picture1, picture2) -> picture2.getTimeOfAdd().compareTo(picture1.getTimeOfAdd()))
                .collect(Collectors.toList());
    }
    ...
    public static List<Picture> sortByLikes(User user) {
        return user.getPictures().stream()
                .sorted((picture1, picture2) -> Integer.compare(picture2.getLikes(), picture1.getLikes()))
                .collect(Collectors.toList());
    }
